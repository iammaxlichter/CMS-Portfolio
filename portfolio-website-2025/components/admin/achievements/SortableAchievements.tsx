// components/admin/achievements/SortableAchievements.tsx
"use client";
...
export default function SortableAchievements({
  initial,
  saveAllAction,
  deleteAction,
}: Props) {
  ...
  // Hydration guard (dnd-kit SSR -> CSR)
  ...
  return (
    <>
      ...
      <form
        action={(fd) => {
          // The form fields (ids[], text-*, published-*) are read by server action.
          startTransition(() => {
            saveAllAction(fd).then(() => setDirty(false));
          });
        }}
        className="space-y-3"
      >
        {/* ids reflect current visual order */}
        {items.map((r) => (
        ...
        {isMounted ? (
          ...
          >
            <SortableContext items={ids} strategy={verticalListSortingStrategy}>
              <ul className="rounded-md border border-neutral-300 divide-y bg-white">
                {items.map((row) => (
                  <SortableRow
                    key={row.id}
                    row={row}
      ...
      {/* HIDDEN DELETE FORMS (one per row) */}
      {deleteAction &&
        items.map((r) => (
          <form
            key={`del-form-${r.id}`}
...
function SortableRow({
  ...
  return (
    <li ref={setNodeRef} style={style} className="p-3 bg-white">
      <div className="flex gap-3 items-start">
        <span
          className={`select-none text-neutral-500 pt-2 ${
            enableDrag ? "cursor-grab hover:text-neutral-800" : ""
          }`}
        ...
        <div className="flex-1 flex items-center gap-3">
          {/* Position for form submission (not trusted server-side, kept for completeness) */}
