...
export default function Slideshow({ data }: { data: SlideshowData & Partial<WithAnim> }) {
    const pathsRaw = data.paths ?? [];
    const paths = useMemo(() => pathsRaw.filter(Boolean).map(toImageSrc), [pathsRaw]);
    ...
    const sizeWrapperStyle: React.CSSProperties = fixedH ? { height: `${fixedH}px` } : { aspectRatio: aspect };

    return (
        <div className="sm:px-8">
            <AnimWrapper anim={data._anim}>
                <figure className={alignClass + " relative w-full"} style={{ maxWidth: `${maxW}px`, marginTop: mt, marginBottom: mb }}>
                    <div
                        className="relative w-full overflow-hidden rounded-xl transition-transform duration-300 ease-out hover:scale-105"
                        style={{
                            padding: pad,
                    ...
                    >
                        <div className="relative" style={sizeWrapperStyle}>
                            <div className="flex h-full w-full transition-transform duration-500" style={{ transform: `translateX(-${i * 100}%)` }}>
                                {paths.map((src, idx) => (
                    ...
                    {paths.length > 1 && (
                        <>
                            ...
                            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
                                {paths.map((_, dot) => (
                                    <span key={dot} className={`h-1.5 w-1.5 rounded-full ${i === dot ? "bg-white" : "bg-white/50"}`} />
                                ))}
                            </div>
                        </>
                    )}
